    ...
    const createOrder = async () => {
        ...
        if (user) {
            ...
            const orderData = {
                ...
                distance: distance,
                pointObtained: parseInt(weight) * 0.2 + 5 * (distance / 2),
                expObtained: Math.round(5 * ((distance / 2) * parseInt(weight))),
                uidOrder: randomUid,
            };
            try {
                await setDoc(doc(db, `users/${user.uid}/history/${randomUid}`), orderData, {merge: true});
                await updateDoc(doc(db, `users/${user.uid}/pointInfo/${user.uid}`), {
                    poin: increment(Math.round(parseInt(weight) * 0.2 + 5 * (distance / 2))),
                    exp: increment(Math.round(5 * ((distance / 2) * parseInt(weight)))),
                });
    ...
    const handleOrder = async () => {
        ...
        setTimeout(() => {
            navigation.navigate("Receipt", {
                ...
                pointObtained: Math.round(parseInt(weight) * 0.2 + 5 * (distance / 2)),
                expObtained: Math.round(5 * ((distance / 2) * parseInt(weight))),
        ...
        <SafeAreaView style={tw`flex-1 h-full`}>
            ...
            {!isOrderClicked ? (
                <View style={styles.container}>
                    ...
                    <BottomSheet ref={bottomSheetRef} index={1} snapPoints={["10%", "50%"]}>
